...
caminho_arquivo = os.path.join(os.path.dirname(os.path.dirname(diretorio_base)), 'dados','0_dados_originais/')

#Inserir arquivo manualmente para testes específicos
arquivo = ""

if (len(arquivo) == 0 ):
    if len(sys.argv) > 1:
        parametro = sys.argv[1]
        print(f"Nome do arquivo recebido foi: {parametro}")
        arquivo = parametro
    else:
        print("Nenhum parâmetro foi passado. Informar o nome do arquivo a ser processado.")
        exit()  # O código para aqui
    

# Leitura do arquivo CSV
if (arquivo == "LOA2024.csv"):
...
# Tratamento para 2001, 2002
if (arquivo == "LOA2001.csv" or arquivo == "LOA2002.csv"):
    df = df.drop(columns=['UG.UF'])
    df = df.rename(columns={'CEP': 'UG.UF'})

# Tratamento para 2002_A_2014
if (arquivo == "LOA2001.csv" or 
    arquivo == "LOA2002.csv" or 
    arquivo == "LOA2003.csv" or 
    arquivo == "LOA2004.csv" or 
    arquivo == "LOA2005.csv" or 
    arquivo == "LOA2006.csv" or 
    arquivo == "LOA2007.csv" or 
    arquivo == "LOA2008.csv" or 
    arquivo == "LOA2009.csv" or 
    arquivo == "LOA2010.csv" or 
    arquivo == "LOA2011.csv" or 
    arquivo == "LOA2012.csv" or 
    arquivo == "LOA2013.csv" or 
    arquivo == "LOA2014.csv"):
...
if(arquivo == "LOA2021.csv" or arquivo == "LOA2023.csv"):
...
if (arquivo == "LOA2015.csv"):
    df_filtrado['Localidade.UF'] = df_filtrado['UF']
    df_filtrado['UG.UF'] = df_filtrado['CEP']

# Tratamento para 2011
if (arquivo == "LOA2011.csv"):
    df_filtrado = df_filtrado.drop(columns=['UG.UF'])
    df_filtrado['UG.UF'] = df_filtrado['CEP']
    df_filtrado = df_filtrado.drop(columns=['CEP'])

# Descomentar para 2023 - por algum motivo veio com formato americano de valores, com '.' e não ','.  
if (arquivo == "LOA2023.csv"):
   df_filtrado['Dotação Inicial'] = df['Dotação Inicial'].astype(str).str.replace('.', ',', regex=False)
   df_filtrado['Autorizado'] = df['Autorizado'].astype(str).astype(str).str.replace('.', ',', regex=False)
   df_filtrado['Empenhado'] = df['Empenhado'].astype(str).astype(str).str.replace('.', ',', regex=False)
   df_filtrado['Despesa Executada'] = df['Despesa Executada'].astype(str).str.replace('.', ',', regex=False)
   df_filtrado['Pago'] = df['Pago'].astype(str).str.replace('.', ',', regex=False)
   df_filtrado['RP Pago'] = df['RP Pago'].astype(str).str.replace('.', ',', regex=False)
...
#Salvar resultado
nome_arquivo_saida = arquivo.replace(".csv","") + '_final_com_regiao.csv'
caminho_e_arquivo_saida = os.path.join(os.path.dirname(os.path.dirname(diretorio_base)) , 'dados','1_dados_regionalizados' , nome_arquivo_saida)
df_filtrado.to_csv(caminho_e_arquivo_saida , index=False , sep="," , quotechar='"')
